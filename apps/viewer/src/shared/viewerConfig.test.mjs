...
test("normalizeViewerSkillsInstallCommand accepts skills add/install and nothing chained on", () => {
  ...
  assert.equal(
    normalizeViewerSkillsInstallCommand("$ npx   skills add   bhaktofmahakal/text-to-cad"),
  ...
  assert.equal(DEFAULT_VIEWER_SKILLS_INSTALL_COMMAND, "npx skills add bhaktofmahakal/text-to-cad");
  ...
  assert.equal(
    normalizeViewerSkillsInstallCommand("npx skills install bhaktofmahakal/text-to-cad"),
    "npx skills install bhaktofmahakal/text-to-cad"
  );
  assert.equal(
...
test("the agent update prompt names the skills command alone, in one short line", () => {
  assert.match(DEFAULT_VIEWER_SKILLS_UPDATE_PROMPT, /npx skills add bhaktofmahakal\/text-to-cad/u);
...
test("normalizeViewerSkillsUpdatePrompt rejects a prompt with no command in it", () => {
  const custom = "Run `npx skills add bhaktofmahakal/text-to-cad` for me.";
  ...
  const legacy = "Run `npx skills install bhaktofmahakal/text-to-cad`.";
