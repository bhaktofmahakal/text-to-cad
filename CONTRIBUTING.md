...
1. Record the current rules (read-only):
   ...
   gh api repos/bhaktofmahakal/text-to-cad/branches/develop/protection
   gh api repos/bhaktofmahakal/text-to-cad/rulesets
   ```
...
4. Protect `main` the way `develop` was protected (done; the classic
   ...
   ```bash
   gh api --method PUT repos/bhaktofmahakal/text-to-cad/branches/main/protection \
     --input - <<'JSON'
...
5. Delete the retired branches once nothing references them:
   ```bash
   gh api --method DELETE repos/bhaktofmahakal/text-to-cad/branches/develop/protection
   git push origin --delete develop release/0.5.0
   git branch -r | sed -n 's#^ *origin/\(release/.*\)#\1#p' | xargs -n1 git push origin --delete
   ```
6. Archive the mirror and drop its secret: `gh repo archive bhaktofmahakal/cad-viewer`
...
8. Re-point PyPI trusted publishing at the new workflow file. A trusted
   ...
   `cadgen` → Publishing, add a publisher for `bhaktofmahakal/text-to-cad`,
